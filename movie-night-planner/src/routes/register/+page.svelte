<script>
	...
	async function handleSubmit(event) {
		...
		if (!username.trim()) {
			error = languageState.t('auth.errorUsernameRequired');
			return;
		}
		...
		if (password.length < 6) {
			error = languageState.t('auth.errorPasswordLength');
			return;
		}

		if (password !== confirmPassword) {
			error = languageState.t('auth.errorPasswordsMismatch');
			return;
		}
...
<section class="mx-auto flex min-h-[calc(100vh-73px)] max-w-6xl items-center justify-center px-4 py-12">
	<div class="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-8 shadow-xl">
		<div class="mb-8 text-center">
			<h1 class="text-3xl font-bold">{languageState.t('auth.createAccountTitle')}</h1>
			<p class="mt-2 text-sm text-slate-400">{languageState.t('auth.registerSubtitle')}</p>
		</div>
		...
		<form onsubmit={handleSubmit} class="space-y-5">
			...
			<div>
				...
				<input
					...
					class="w-full rounded-lg border border-slate-700 bg-slate-950 px-4 py-3 text-white outline-none focus:border-indigo-500"
					placeholder={languageState.t('auth.passwordMinPlaceholder')}
				/>
			</div>

			<div>
				<label for="confirmPassword" class="mb-2 block text-sm font-medium text-slate-300">
					{languageState.t('auth.confirmPassword')}
				</label>
				...
				<input
					...
					autocomplete="new-password"
					class="w-full rounded-lg border border-slate-700 bg-slate-950 px-4 py-3 text-white outline-none focus:border-indigo-500"
					placeholder={languageState.t('auth.confirmPasswordPlaceholder')}
				/>
			</div>
			...
			>
				{loading ? languageState.t('auth.registerLoading') : languageState.t('app.register')}
			</button>
		</form>

		<p class="mt-6 text-center text-sm text-slate-400">
			{languageState.t('auth.hasAccount')}
			<a href={resolve('/login')} class="font-medium text-indigo-400 hover:text-indigo-300">
				{languageState.t('auth.loginTitle')}
			</a>
		</p>
	</div>
</section>
